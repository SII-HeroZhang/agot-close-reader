...
- **PDF 对照阅读**：直接阅读仓库中的 755 页原始 PDF，并按页查看对应精读；章节段落可一键跳到原页。
...
打开顶部导航中的 [PDF 阅读器](https://sii-herozhang.github.io/agot-close-reader/pdf/)：

- 左侧直接显示完整 PDF，右侧列出当前页对应的精读段落；
- 输入页码或使用左右按钮翻页，右侧解读会同步更新；
- 点击右侧条目可以进入该段完整精读；
- 在任意章节中点击段落顶部的 **PDF p.X**，可以打开“原页 + 对应解读”分栏窗口；
- 如果浏览器无法内嵌显示 PDF，可以点击“在新标签页打开当前页”。

站内 PDF 与精读使用同一套页码映射。也可以临时选择另一份本地 PDF；本地版本只会保存在当前浏览器中。
...
│   ├── public/
│   │   ├── og.png                 # 社交分享卡片
│   │   └── agot-original.pdf      # 站内 PDF 对照阅读源文件
│   ├── build.py                   # 静态网站生成器
...
本站为非商业语言学习与文本分析项目。原著文字和 PDF 版权归 George R. R. Martin 及相关权利人所有。公开发布完整原文及 PDF 存在版权风险；免责声明不构成授权，也不能消除该风险。如有权利问题，请通过 [GitHub Issues](https://github.com/SII-HeroZhang/agot-close-reader/issues/new) 联系处理或请求下架。
